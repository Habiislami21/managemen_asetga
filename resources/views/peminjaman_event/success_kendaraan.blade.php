...
<head>
    ...
    <title>Form Kendaraan Berhasil Dibuat</title>
...
<body>
    <div class="success-container">
        ...
        <p class="text-muted mb-0">Dokumen Word peminjaman kendaraan telah siap diunduh.</p>

        <div class="info-box">
            <div><strong>No. Peminjaman:</strong> {{ $peminjaman->nomor_peminjaman }}</div>
            <div><strong>Peminjam:</strong> {{ $peminjaman->nama_peminjam }}</div>
            <div><strong>Kendaraan:</strong> {{ $peminjaman->nama_kendaraan }} ({{ $peminjaman->nomor_plat }})</div>
            <div><strong>Kegiatan:</strong> {{ $peminjaman->nama_kegiatan }}</div>
        </div>

        <a href="{{ route('peminjaman-event.kendaraan.download', $peminjaman) }}" class="btn-download">
            <i class="fas fa-download me-2"></i>Unduh Form Word (.docx)
        </a>

        <div class="mt-4">
            <a href="{{ route('peminjaman-event.kendaraan.create') }}" class="text-decoration-none me-3" style="color: var(--primary);">
                Buat Form Baru
            </a>
