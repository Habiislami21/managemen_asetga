...
<body class="bg-gray-100">
    <div class="container mx-auto p-6" id="app">
        <div class="bg-white rounded-lg shadow-lg p-6 mb-6">
            <h1 class="text-2xl font-bold mb-4">Edit Pengajuan Rutin</h1>
            
            <form action="{{ route('ajuan-rutin.update-batch') }}" method="POST">
                @csrf
                <input type="hidden" name="old_nama_spa" value="{{ $namaSpa }}">
                <input type="hidden" name="old_tanggal_ajuan" value="{{ $tanggalAjuan }}">
                <input type="hidden" name="old_divisi_id" value="{{ $divisiId }}">
                
                <div class="mb-6 grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-gray-700 mb-2" for="nama_spa">Nama SPA:</label>
                        <input type="text" name="nama_spa" id="nama_spa" class="w-full border rounded-md p-2" value="{{ $namaSpa }}" required>
                    </div>
                    <div>
                        ...
                        <select name="divisi_id" id="divisi_id" class="w-full border rounded-md p-2" required>
                            ...
                            <option value="{{ $divisi->id }}"
                                {{ (old('divisi_id') ?? $divisiId) == $divisi->id ? 'selected' : '' }}>
                                {{ $divisi->divisi}}
                            </option>
                        @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="text" name="nomor_telp" id="nomor_telp" class="w-full border rounded-md p-2" value="{{ $items->first()->nomor_telp }}" required>
                    </div>
                </div>
        ...
        setup() {
            ...
            const initializeItems = () => {
                ...
                if (existingItems && existingItems.length > 0) {
                    items.value = existingItems.map(item => ({
